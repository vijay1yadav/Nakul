...
const NetworkCheckIcon = bundleIcon(NetworkCheckFilled, NetworkCheckRegular);
...
const DDoSProtectionCostSection = ({ ddosProtectionCostData, granularity, setGranularity }) => {
    const styles = useStyles();
    const {
        monthlyCost = 0,
        protectedIPs = 0,
        lastUpdated = 'N/A',
        // Assume a more detailed breakdown by protected resource
        costBreakdownByResource = [],
        // Assume data for a cost trend chart
        costTrend = { labels: [], data: [] },
    } = ddosProtectionCostData;
    ...
    if (!ddosProtectionCostData || Object.keys(ddosProtectionCostData).length === 0) {
        return <Spinner label="Loading DDoS Protection cost data..." />;
    }
    ...
    return (
        <div className={styles.root}>
            <div className={styles.summaryGrid}>
                ...
                <MetricCard icon={<NetworkCheckIcon />} title="Protected IPs / VNets" value={protectedIPs} />
            </div>
            ...
            <div className={styles.mainGrid}>
                ...
                <Card>
                    <CardHeader header={<Subtitle1>Cost Breakdown by Protected Resource</Subtitle1>} />
                    <div className={styles.tableContainer}>
                        <Table aria-label="DDoS Protection Cost Breakdown Table">
                            <TableHeader>
                                <TableRow>
                            ...
                            <TableBody>
                                {costBreakdownByResource.map((item, index) => (
                                    <TableRow key={index}>
...
export default DDoSProtectionCostSection;
