...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        const costPromises = subscriptions.map(sub =>
            fetchCostData(accessToken, sub.subscriptionId, startDate, endDate, ['Azure Key Vault'])
        ...
        console.log('Key Vault cost data:', keyVaultCostData);
        res.json(keyVaultCostData);
    } catch (error) {
        console.error('Error in /keyVaultCost:', error.message);
        res.status(500).json({ error: 'Failed to fetch Key Vault cost data' });
    }
});
